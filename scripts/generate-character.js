$(document).ready(function() {
    ...
    const charactergeneratorobjects = {

        heading: [
            "I'm going to roll the stats for",
            "Check this out, I'm going to be",
            "You think your character is cool? My character is",
            "I'm going to roleplay",
            "I'm going to be",
            "I'm going to play",
            "I'm thinking of rolling up",
            "Watch out, I'm going to be",
            "Guess what, I'm creating",
            "I'm definitely playing",
            "My next character is going to be",
            "Prepare to meet",
            "You won't believe this, but I'm making",
            "I'm building my stats for",
            "I'm really excited to play",
            "My next role is definitely"
        ],
        ...
        response: [
            ...
            "That seems boring",
            "I'm really not into that idea",
            "Are you sure that's the right choice?",
            "I was hoping for something different",
            "That doesn't fit the party",
            "I'm going to pass on that one",
            "That feels way too common"
        ],

        template: [
            "@indefinitearticle @adjective @race @background, @subclass @dclass from @location, who @backstory.",
        ],
        ...
        // Class races from the D&D 5e Player's Handbook
        // race: [
        //     "Dragonborn",
        //     "Dwarf",
        //     "Elf",
        ...
        subclasses: {
            ...
            Cleric: ["Knowledge Domain", "Life Domain", "Light Domain", "Nature Domain", "Tempest Domain", "Trickery Domain", "War Domain", "Arcana Domain", "Forge Domain", "Grave Domain", "Order Domain", "Peace Domain", "Twilight Domain"],
            ...
            Monk: ["Way of the Open Hand", "Way of Shadow", "Way of the Four Elements", "Way of the Drunken Master", "Way of the Kensei", "Way of the Sun Soul", "Way of the Astral Self", "Way of Mercy"],
            Paladin: ["Oath of Devotion", "Oath of the Ancients", "Oath of Vengeance", "Oath of the Crown", "Oath of Conquest", "Oath of Redemption", "Oath of Glory", "Oath of the Watchers"],
            ...
            Sorcerer: ["Draconic Bloodline", "Wild Magic", "Storm Sorcery", "Divine Soul", "Shadow Magic", "Aberrant Mind", "Clockwork Soul"],
            Warlock: ["The Archfey", "The Fiend", "The Great Old One", "The Hexblade", "The Celestial", "The Fathomless", "The Genie", "Undead"],
            Wizard: ["School of Abjuration", "School of Conjuration", "School of Divination", "School of Enchantment", "School of Evocation", "School of Illusion", "School of Necromancy", "School of Transmutation", "Bladesinging", "War Magic"],
    ...
    function generateCharacter() {
        // Select random elements based on the template
        const heading = getRandomItem(charactergeneratorobjects.heading);
        const adjective = getRandomItem(charactergeneratorobjects.adjective);
        const race = getRandomItem(charactergeneratorobjects.race);
        const background = getRandomItem(charactergeneratorobjects.background);
        const dclass = getRandomItem(charactergeneratorobjects.dclass);
        const subclass = getRandomItem(charactergeneratorobjects.subclasses[dclass]);
        const location = getRandomItem(charactergeneratorobjects.location);
        const backstory = getRandomItem(charactergeneratorobjects.backstory);
        const response = getRandomItem(charactergeneratorobjects.response);

        // Function to get the indefinite article for a word
        function getIndefiniteArticle(adjective) {
            // Regular expression to check if the word starts with a vowel sound or a silent 'h'
            var vowelRegex = /^[aeiouAEIOU]/; // Matches words starting with vowels
            var silentHRegex = /^hon|^hour/i;  // Matches words starting with a silent 'h' (e.g., "honorable", "hour")
        
            if (silentHRegex.test(adjective)) {
                return 'An';
            } else if (vowelRegex.test(adjective)) {
                return 'An';
            } else {
                return 'A';
            }
        ...
        const characterTemplate = charactergeneratorobjects.template[0]
            .replace('@indefinitearticle', getIndefiniteArticle(adjective))
            .replace('@adjective', adjective)
            .replace('@race', race)
            .replace('@background', background)
            .replace('@subclass', subclass)
            .replace('@dclass', dclass)
            .replace('@location', location)
            .replace('@backstory', backstory);

        // Update the HTML
        $('#heading').text(heading);
        $('#characterTemplate').text(characterTemplate);
        $('#response').text(response);

        // Change the button text to "Regenerate" if <p> elements have data
        if ($('#heading').text().trim() !== '' && $('#characterTemplate').text().trim() !== '') {
            $('#response').text(response);
        }
    }
